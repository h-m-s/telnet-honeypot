...
class HoneyTelnetServer(TelnetServer):
        def __init__(self, port=7777, address='', logger=None):
            ...
            self.SERVER_RUN = 1
            self.logger = logger
            super().__init__(port, address, self.on_connect, self.on_disconnect)
            self.client_list = []
        ...
        def on_connect(self, client):
                ...
                """
                self.logger.info("Opened connection to {}".format(client.addrport()))
        ...
        def on_disconnect(self, client):
                ...
                for disconnections
                """
                self.logger.info("Lost connection to {}".format(client.addrport()))
                client.cleanup_container()
                self.client_list.remove(client)
        ...
        def kick_idle(self):
                ...
                for client in self.client_list:
                        if client.idle() > IDLE_TIMEOUT:
                                self.logger.info("Kicking idle client from {}".format(client.addrport()))
                                client.active = False
        ...
        def login_screen(self, client, msg):
                ...
                if msg != "":
                        if not client.username:
                                ...
                                self.logger.info("{} logged in as {}-{}".format(client.addrport(), client.username, client.password))
